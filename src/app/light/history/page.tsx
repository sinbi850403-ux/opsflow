        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-bold text-slate-900">
              {totalUploadCount.toLocaleString("ko-KR")}
            </p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm text-slate-500">전체 행 합계</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">
              {totalRowsSum.toLocaleString("ko-KR")}
            </p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm text-slate-500">미리보기 행 합계</p>
            <p className="mt-2 text-2xl font-bold text-sky-700">
              {previewRowsSum.toLocaleString("ko-KR")}
            </p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm text-slate-500">헤더 수 합계</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">
              {totalHeaderCount.toLocaleString("ko-KR")}
            </p>
          </div>
        </section>

        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          <p className="text-sm text-slate-500">최근 업로드 시각</p>
              ...
              <table className="min-w-full border-collapse text-sm">
                <thead className="bg-slate-50">
                  <tr className="border-b border-slate-200 text-left text-slate-600">
                    ...
                    <th className="px-4 py-3 text-right font-semibold">미리보기 행</th>
                    <th className="px-4 py-3 text-right font-semibold">헤더 수</th>
                  </tr>
                </thead>
                <tbody>
